...
} = useApi<DimProjeto[]>('/api/dim/projetos')

const {
  data: tarefas,
  status: tarefasStatus
} = useApi<DimTarefa[]>('/api/dim/tarefas')

const {
  data: execucoes,
  status: execucoesStatus
} = useApi<FatoExecucao[]>(
...
const skProjetosFiltrados = computed(() =>
  new Set(
    projetosFiltrados.value.map(
      p => p.id_projeto
    )
  )
)
...
const execucoesFiltradas = computed(() =>
  execucoes.value?.filter(e =>
    skProjetosFiltrados.value.has(
      e.sk_projeto
    )
  ) ?? []
)
...
const totalCompras = computed(() =>
  comprasFiltradas.value.reduce(
    (acc, c) =>
      acc + Number(c.valor_total_pedido),
    0
  )
)
...
const stats = computed(() => [
  ...
  {
    label: 'Tarefas',
    value: tarefasFiltradas.value.length,
  ...
  },
  {
    ...
    value: totalCompras.value.toLocaleString(
      'pt-BR',
      {
        style: 'currency',
        currency: 'BRL'
...
const projetoNomeMap = computed(() => {
  if (!projetosFiltrados.value.length) {
    return new Map<string, string>()
  }

  return new Map(
    projetosFiltrados.value.map(p => [
      p.id_projeto,
      p.nome_projeto
    ])
  )
...
const horasPorProjeto = computed(() =>
  groupBy(
    execucoesFiltradas.value,
    'horas_trabalhadas'
  )
)

const comprasPorProjeto = computed(() =>
  groupBy(
    ...
    <template #body>
      ...
      <div class="mb-4">
        <div class="w-full max-w-sm">
          <UInput
            v-model="pesquisa"
            placeholder="Pesquisar projeto ou tarefa..."
      ...
      <div
        v-if="pesquisa && tarefasFiltradas.length"
        class="mt-6"
      >
        <UCard>
          <template #header>
          ...
          <div class="space-y-3">
            <div
              v-for="tarefa in tarefasFiltradas.slice(0, 10)"
      ...
      >
        <UCard>
          ...
          <template v-if="loading">
            <div
              class="flex h-72 items-end gap-3 px-4"
            >
              <USkeleton
                v-for="i in 5"
                :key="i"
        ...
        <UCard>
          ...
          <template v-if="loading">
            <div
              class="flex h-72 flex-col justify-center gap-3 px-4"
            >
